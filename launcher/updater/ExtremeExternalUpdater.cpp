...
#include "ExtremeExternalUpdater.h"
...
class ExtremeExternalUpdater::Private {
   public:
    QDir appDir;
    QDir dataDir;
    QTimer updateTimer;
...
ExtremeExternalUpdater::ExtremeExternalUpdater(QWidget* parent, const QString& appDir, const QString& dataDir)
{
    priv = new ExtremeExternalUpdater::Private();
    priv->appDir = QDir(appDir);
    priv->dataDir = QDir(dataDir);
    auto settings_file = priv->dataDir.absoluteFilePath("extremelauncher_update.cfg");
...
ExtremeExternalUpdater::~ExtremeExternalUpdater()
{
...
void ExtremeExternalUpdater::checkForUpdates()
{
    checkForUpdates(true);
}

void ExtremeExternalUpdater::checkForUpdates(bool triggeredByUser)
{
...
bool ExtremeExternalUpdater::getAutomaticallyChecksForUpdates()
{
    return priv->autoCheck;
}

double ExtremeExternalUpdater::getUpdateCheckInterval()
{
    return priv->updateInterval;
}

bool ExtremeExternalUpdater::getBetaAllowed()
{
    return priv->allowBeta;
}

void ExtremeExternalUpdater::setAutomaticallyChecksForUpdates(bool check)
{
...
void ExtremeExternalUpdater::setUpdateCheckInterval(double seconds)
{
...
void ExtremeExternalUpdater::setBetaAllowed(bool allowed)
{
...
void ExtremeExternalUpdater::resetAutoCheckTimer()
{
...
void ExtremeExternalUpdater::connectTimer()
{
    connect(&priv->updateTimer, &QTimer::timeout, this, &ExtremeExternalUpdater::autoCheckTimerFired);
}

void ExtremeExternalUpdater::disconnectTimer()
{
    disconnect(&priv->updateTimer, &QTimer::timeout, this, &ExtremeExternalUpdater::autoCheckTimerFired);
}

void ExtremeExternalUpdater::autoCheckTimerFired()
{
    qDebug() << "Auto update Timer fired";
    checkForUpdates(false);
}

void ExtremeExternalUpdater::offerUpdate(const QString& version_name, const QString& version_tag, const QString& release_notes)
{
...
void ExtremeExternalUpdater::performUpdate(const QString& version_tag)
{
    QProcess proc;
    auto exe_name = QStringLiteral("%1_updater").arg(BuildConfig.LAUNCHER_APP_BINARY_NAME);
#if defined Q_OS_WIN32
