...
#include "updater/extremeupdater/UpdaterDialogs.h"
...
void appDebugOutput(QtMsgType type, const QMessageLogContext& context, const QString& msg)
{
    ...
    ExtremeUpdaterApp* app = static_cast<ExtremeUpdaterApp*>(QCoreApplication::instance());
...
ExtremeUpdaterApp::ExtremeUpdaterApp(int& argc, char** argv) : QApplication(argc, argv)
{
#if defined Q_OS_WIN32
    ...
    parser.setApplicationDescription(QObject::tr("An auto-updater for Extreme Launcher"));
    ...
    QString extreme_executable = FS::PathCombine(applicationDirPath(), BuildConfig.LAUNCHER_APP_BINARY_NAME);
#if defined Q_OS_WIN32
    extreme_executable.append(".exe");
#endif

    if (!QFileInfo(extreme_executable).isFile()) {
        showFatalErrorMessage(tr("Unsupported Installation"), tr("The updater can not find the main executable."));
    }

    m_extremeExecutable = extreme_executable;

    auto extreme_update_url = parser.value("update-url");
    if (extreme_update_url.isEmpty())
        extreme_update_url = BuildConfig.UPDATER_GITHUB_REPO;

    m_extremeRepoUrl = QUrl::fromUserInput(extreme_update_url);
    ...
    auto version = parser.value("extreme-version");
    if (!version.isEmpty()) {
        if (version.contains('-')) {
        ...
        m_extremeVersionMajor = version_parts.takeFirst().toInt();
        m_extremeVersionMinor = version_parts.takeFirst().toInt();
    }
    ...
    if (!dirParam.isEmpty()) {
        // the dir param. it makes extreme launcher data path point to whatever the user specified
        // on command line
    ...
    m_updateLogPath = FS::PathCombine(m_dataPath, "logs", "extreme_launcher_update.log");
    ...
    auto marker_file_path = QDir(m_rootPath).absoluteFilePath(".extreme_launcher_updater_unpack.marker");
    ...
    if (marker_file.exists()) {
        ...
        QMetaObject::invokeMethod(this, &ExtremeUpdaterApp::loadReleaseList, Qt::QueuedConnection);
    }
}

ExtremeUpdaterApp::~ExtremeUpdaterApp()
{
...
void ExtremeUpdaterApp::fail(const QString& reason)
{
...
void ExtremeUpdaterApp::abort(const QString& reason)
{
    qCritical() << qPrintable(reason);
    m_status = Aborted;
    exit(2);
...
void ExtremeUpdaterApp::showFatalErrorMessage(const QString& title, const QString& content)
{
    m_status = Failed;
...
void ExtremeUpdaterApp::run()
{
    qDebug() << "found" << m_releases.length() << "releases on github";
    qDebug() << "loading exe at " << m_extremeExecutable;
    ...
    if (!loadExtremeVersionFromExe(m_extremeExecutable)) {
        m_extremeVersion = BuildConfig.printableVersionString();
        m_extremeVersionMajor = BuildConfig.VERSION_MAJOR;
        m_extremeVersionMinor = BuildConfig.VERSION_MINOR;
        m_prsimVersionChannel = BuildConfig.VERSION_CHANNEL;
        m_extremeGitCommit = BuildConfig.GIT_COMMIT;
    }
    m_status = Succeeded;

    qDebug() << "Executable reports as:" << m_extremeBinaryName << "version:" << m_extremeVersion;
    qDebug() << "Version major:" << m_extremeVersionMajor;
    qDebug() << "Version minor:" << m_extremeVersionMinor;
    qDebug() << "Version channel:" << m_prsimVersionChannel;
    qDebug() << "Git Commit:" << m_extremeGitCommit;
    ...
    if (m_isFlatpak) {
        showFatalErrorMessage(tr("Updating flatpack not supported"), tr("Actions outside of checking if an update is available are not "
                                                                        "supported when running the flatpak version of Extreme Launcher."));
        return;
    }
    if (m_isAppimage) {
        bool result = true;
...
void ExtremeUpdaterApp::moveAndFinishUpdate(QDir target)
{
    ...
    if (error) {
        ...
        auto fail_marker = FS::PathCombine(m_dataPath, ".extreme_launcher_update.fail");
        FS::copy(m_updateLogPath, fail_marker).overwrite(true)();
    } else {
        logUpdate(tr("Update succeed."));
        auto success_marker = FS::PathCombine(m_dataPath, ".extreme_launcher_update.success");
        FS::copy(m_updateLogPath, success_marker).overwrite(true)();
    }
    auto update_lock_path = FS::PathCombine(m_dataPath, ".extreme_launcher_update.lock");
...
void ExtremeUpdaterApp::printReleases()
{
...
QList<GitHubRelease> ExtremeUpdaterApp::nonDraftReleases()
{
...
QList<GitHubRelease> ExtremeUpdaterApp::newerReleases()
{
    QList<GitHubRelease> newer;
    for (auto rls : nonDraftReleases()) {
        if (rls.version > m_extremeVersion)
            newer.append(rls);
    }
    return newer;
}

GitHubRelease ExtremeUpdaterApp::selectRelease()
{
    ...
    SelectReleaseDialog dlg(Version(m_extremeVersion), releases);
...
QList<GitHubReleaseAsset> ExtremeUpdaterApp::validReleaseArtifacts(const GitHubRelease& release)
{
...
GitHubReleaseAsset ExtremeUpdaterApp::selectAsset(const QList<GitHubReleaseAsset>& assets)
{
...
void ExtremeUpdaterApp::performUpdate(const GitHubRelease& release)
{
...
QFileInfo ExtremeUpdaterApp::downloadAsset(const GitHubReleaseAsset& asset)
{
...
bool ExtremeUpdaterApp::callAppImageUpdate()
{
...
void ExtremeUpdaterApp::clearUpdateLog()
{
    FS::deletePath(m_updateLogPath);
}

void ExtremeUpdaterApp::logUpdate(const QString& msg)
{
...
void ExtremeUpdaterApp::performInstall(QFileInfo file)
{
    ...
    auto changelog_path = FS::PathCombine(m_dataPath, ".extreme_launcher_update.changelog");
    FS::write(changelog_path, m_install_release.body.toUtf8());

    logUpdate(tr("Updating from %1 to %2").arg(m_extremeVersion).arg(m_install_release.tag_name));
    if (m_isPortable || file.fileName().endsWith(".zip") || file.fileName().endsWith(".tar.gz")) {
        write_lock_file(update_lock_path, QDateTime::currentDateTime(), m_extremeVersion, m_install_release.tag_name, m_rootPath, m_dataPath);
...
void ExtremeUpdaterApp::unpackAndInstall(QFileInfo archive)
{
    logUpdate(tr("Backing up install"));
    backupAppDir();

    if (auto loc = unpackArchive(archive)) {
        auto marker_file_path = loc.value().absoluteFilePath(".extreme_launcher_updater_unpack.marker");
...
void ExtremeUpdaterApp::backupAppDir()
{
    auto manifest_path = FS::PathCombine(m_rootPath, "manifest.txt");
    QFileInfo manifest(manifest_path);
    ...
    if (file_list.isEmpty()) {
        ...
        if (BuildConfig.BUILD_ARTIFACT.toLower().contains("linux")) {
            file_list.append({ "ExtremeLauncher", "bin", "share", "lib" });
        } else {  // windows by process of elimination
            file_list.append({
                "jars",
                "extremelauncher.exe",
                "extremelauncher_filelink.exe",
                "extremelauncher_updater.exe",
                "qtlogging.ini",
                "imageformats",
                "iconengines",
                "platforms",
    ...
    auto backup_dir = FS::PathCombine(
        ...
        QStringLiteral("backup_") +
            QString(m_extremeVersion).replace(QRegularExpression("[" + QRegularExpression::escape("\\/:*?\"<>|") + "]"), QString("_")) + "-" +
            m_extremeGitCommit);
    FS::ensureFolderPathExists(backup_dir);
    auto backup_marker_path = FS::PathCombine(m_dataPath, ".extreme_launcher_update_backup_path.txt");
...
std::optional<QDir> ExtremeUpdaterApp::unpackArchive(QFileInfo archive)
{
    auto temp_extract_path = FS::PathCombine(m_dataPath, "extreme_launcher_update_release");
...
bool ExtremeUpdaterApp::loadExtremeVersionFromExe(const QString& exe_path)
{
    QProcess proc = QProcess();
    ...
    if (first_parts.length() < 2)
        return false;
    m_extremeBinaryName = first_parts.takeFirst();
    auto version = first_parts.takeFirst().trimmed();
    m_extremeVersion = version;
    if (version.contains('-')) {
        auto index = version.indexOf('-');
        m_prsimVersionChannel = version.mid(index + 1);
        version = version.left(index);
    ...
    m_extremeVersionMajor = version_parts.takeFirst().toInt();
    m_extremeVersionMinor = version_parts.takeFirst().toInt();
    m_extremeGitCommit = lines.takeFirst().simplified();
    return true;
}

void ExtremeUpdaterApp::loadReleaseList()
{
    auto github_repo = m_extremeRepoUrl;
...
void ExtremeUpdaterApp::downloadReleasePage(const QString& api_url, int page)
{
    int per_page = 30;
    ...
    connect(download.get(), &Net::Download::failed, this, &ExtremeUpdaterApp::downloadError);
...
int ExtremeUpdaterApp::parseReleasePage(const QByteArray* response)
{
...
GitHubRelease ExtremeUpdaterApp::getLatestRelease()
{
...
bool ExtremeUpdaterApp::needUpdate(const GitHubRelease& release)
{
    auto current_ver = Version(QString("%1.%2").arg(QString::number(m_extremeVersionMajor)).arg(QString::number(m_extremeVersionMinor)));
    return current_ver < release.version;
}

void ExtremeUpdaterApp::downloadError(QString reason)
{
